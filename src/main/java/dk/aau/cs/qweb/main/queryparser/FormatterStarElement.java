...
public FormatterStarElement(IndentedWriter out, SerializationContext context) {
    super(out, context) ;
}

public static void format(IndentedWriter out, SerializationContext cxt, Element el) {
    FormatterStarElement fmt = new FormatterStarElement(out, cxt) ;
    fmt.startVisit() ;
    el.visit(fmt) ;
    fmt.finishVisit() ;
}
...
public static String asString(Element el) {
    ...
    FormatterStarElement.format(b, cxt, el) ;
    return b.toString() ;
}
